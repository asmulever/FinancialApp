...
public abstract class SqlCrudRepository<T> : IGenericCrudRepository<T> where T : class, IEntityBase
{
    #region "Variables"
    ...
    #region "Constructor"
    public SqlCrudRepository(IDbConnection dbConnection)
    {
  ...
  public async Task<int> Ins(T entity)
{
    ...
    var insertedId = await _dbConnection.QuerySingleAsync<int>(sql, propertyValues);
    ...
    public async Task<bool> Upd(int id, T entity)
    {
        var entityType = typeof(T);
        var idProperty = entityType.GetProperty("Id");
